...
num_classes = 4  # 3 classes (with_mask, without_mask, mask_weared_incorrect) + background
...
train_dataset = MaskedFaceDataset('MaskedFace/train', img_transforms=torchvision.transforms.ToTensor())
...
for epoch in range(num_epochs):
    ...
    print(f"Epoch [{epoch+1}/{num_epochs}], Training Loss: {running_loss/len(train_loader):.4f}, Validation MAPE: {mape:.4f}%")
